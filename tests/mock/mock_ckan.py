# A test server. Adapted from
# https://github.com/ckan/ckanapi/blob/master/ckanapi/tests/mock/mock_ckan.py
...
def mock_ckan(environ, start_response):
    ...
    if (environ['PATH_INFO'] == '/api/action/package_create'
        or environ['PATH_INFO'] == '/api/action/package_delete'):
        clen = int(environ.get('CONTENT_LENGTH', '0'))
        if clen != 0:
            body = json.loads(environ['wsgi.input'].read(clen))
        start_response(status, headers)
        return [json.dumps({
            "help": "none",
            "success": True,
